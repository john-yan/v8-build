...
$(BASE_IMAGE): Dockerfile.base
	$(DOCKER_BUILD) -t quay.io/mfarazma/rt-nodejs-build-$@-$(MACHINE_ARCH) -f $< .
...
$(TESTS): %:
	docker pull quay.io/mfarazma/rt-nodejs-build-$@-img-$(MACHINE_ARCH)
	docker run -e "NPROC=$(NPROC)" quay.io/mfarazma/rt-nodejs-build-$@-img-$(MACHINE_ARCH) $(features)
	docker system prune -f

define PUSH
	docker push quay.io/mfarazma/rt-nodejs-build-$(1)-$(MACHINE_ARCH);
endef
